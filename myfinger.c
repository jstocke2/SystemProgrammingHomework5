/* myfinger.c - a client for a remote finger listing service
...
main(int ac, char *av[])
{
	...
	if ( ac != 3 ) 
	{
		printf("Wrong CLI argument count\n");
		exit(1);
	}
	...
	hp = gethostbyname( av[2] );            /* lookup host's ip #   */
	if (hp == NULL) 
		oops(av[1]);            	/* or die               */
	...
	if ( write(sock_id, av[1], strlen(av[1])) == -1)
		oops("write");
	if ( write(sock_id, "\n", 1) == -1 )
		oops("write");
